...
-- 메인 쿼리 실행 중에 서브쿼리를 실행해서 결과값을 메인쿼리에 전달하는 방식
...
-- 조건, 현재 재직자만 (관리자의 코드는 dept_no = d005, 재직 중인건 to_date = 9999-01-01)
select emp_no
from dept_manager
where to_date = '9999-01-01'
AND dept_no = 'd005';

select * from employees where emp_no =(select emp_no from dept_manager where to_date='9999-01-01' AND dept_no='d005');
...
select emp_no
from salaries where to_date='9999-01-01' and salary > (select avg(salary) from salaries where to_date = '9999-01-01');

#평균 급여보다 많이 받는 재직자의 직원정보
select *
from employees where emp_no in (select emp_no
                                from salaries where to_date='9999-01-01' and salary > (select avg(salary) from salaries where to_date = '9999-01-01'));
